...
import { useWebsiteInfo } from "../Providers/WebsiteInfoProvider";
...
export default function WhatsAppContact({className}: { className?: string }) {
  ...
  return (
    <a href={info.whatsapp ?? "/"} className={className}>
      <Button
        className="flex gap-2 w-full bg-green-400 text-white hover:bg-green-500 hover:text-white"
        variant={"outline"}
      >
        <svg
